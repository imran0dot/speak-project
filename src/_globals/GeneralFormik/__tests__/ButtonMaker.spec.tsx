...
describe('ButtonMaker tests', () => {
  ...
  it('rendering and submitting a basic Formik form', async () => {
    ...
    it('submitButtonEl should render', () => {
        const { container  } = render(<GeneralFormik schema={schema2} submitHandler={()=>{jest.fn()}} />)
        //const submitButtonEl = container.querySelector(`button[name="submit"]`);
        expect(screen.getByRole('button', {name: /submit/i})).toBeDefined();
    })
    
    it('resetButtonEl should render', () => {
        const { container  } = render(<GeneralFormik schema={schema2} submitHandler={()=>{jest.fn()}} />)
        //const submitButtonEl = container.querySelector(`button[name="submit"]`);
    ...
    /*
        it('reset button should not be disabled',()=>{
          const button = renderer.create(<ButtonMaker schema={schema2} submitHandler={()=>{jest.fn()}}  />);
          expect(button.toJSON()[0].props.disabled).toBe(false)
        })
        it('submit button should not be disabled',()=>{
          const button = renderer.create(<ButtonMaker schema={schema2} submitHandler={()=>{jest.fn()}}  />);
          expect(button.toJSON()[1].props.disabled).toBe(false)
        })
    */
