...
export default function EnhancedTable(props) {
  console.log("xxxxxxxxxxxxxxxxx------props", props)
  ...
  React.useEffect(() => {
    ...
    if(previousProps.data !== props.data) {
      ...
      console.log("CHANGEINPROP", props.data)
  ...
  const renderCells = (labelId, row) => {
    console.log("xxxxxxxx---------row", row)

    let stack = [];
    for (let i = 0; i < Object.keys(row).length; ++i) {
      console.log("xxx-", Object.keys(row)[i])
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar selected={selected} numSelected={selected.length} otherActions={props.data.otherActions} handleAction={props.handleAction} />
        <TableContainer>
          <Table
